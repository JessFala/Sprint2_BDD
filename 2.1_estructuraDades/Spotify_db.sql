...
CREATE TABLE Playlist_activa(
    ...
    llista_colaborativa ENUM('yes', 'no') NOT NULL,
	id_usuarx INT NOT NULL,
    id_playlist_esborrada INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
...
CREATE TABLE Artistxs_prefer(
  artistx_prefer_id INT NOT NULL AUTO_INCREMENT,
  id_usuarx INT NOT NULL,
  id_artista INT NOT NULL,
  PRIMARY KEY (artistx_prefer_id),
  INDEX Artistxs_prefer_user1_idx(id_usuarx ASC) VISIBLE,
  INDEX Artistxs_prefer_artist1_idx (id_artista ASC) VISIBLE,
  CONSTRAINT Artistxs_prefer_user1_idx
    FOREIGN KEY (id_usuarx)
    REFERENCES Usuarix (id_usuarx)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT Artistxs_prefer_artistx1
    FOREIGN KEY (id_artista)
    REFERENCES Artistes (id_artista)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;